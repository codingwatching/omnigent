...
# Grant/revoke/reassign/admin-flag writes evict, and a generation counter stops
# an in-flight reader from re-storing a pre-commit positive on top of that
# eviction, so once a write returns this instance serves no stale decision.
# Across replicas there is no invalidation broadcast, so a revoke can be up to
# the TTL late elsewhere; that window is LEVEL_EDIT only — the destructive
# stop/kill path re-gates LEVEL_OWNER separately, and a deleted session still
# 404s on its uncached conversation read. Entries are keyed by (conversation_id, user_id):
...
# Live permission stores in this process, so a write made through a *different*
# object on the same rows can drop the stale decisions it invalidates.
# ``users`` / ``session_permissions`` have a second writer in accounts mode
# (:class:`omnigent.server.accounts_store.SqlAlchemyAccountStore` — its
# ``set_admin`` demotes and its ``delete_user`` removes every grant), and that
# store holds no reference to this one. Weak refs so a discarded store (tests
# build one per case) is collected normally.
_LIVE_PERMISSION_STORES: weakref.WeakSet[SqlAlchemyPermissionStore] = weakref.WeakSet()


def invalidate_resolve_access_caches() -> None:
    """Drop every live ``resolve_access`` cache in this process.

    For writers outside :class:`SqlAlchemyPermissionStore` that change the
    ``users`` / ``session_permissions`` rows it caches. Call *after* the write
    commits, so a concurrent reader cannot re-cache the pre-commit state.
    A no-op when no store has been constructed.
    """
    for store in list(_LIVE_PERMISSION_STORES):
        store._invalidate_resolve_cache_all()
...
class SqlAlchemyPermissionStore(PermissionStore):
    ...
    def __init__(self, storage_location: str) -> None:
        ...
        _LIVE_PERMISSION_STORES.add(self)
    ...
    def reassign_user_grants(self, from_user_id: str, to_user_id: str) -> int:
        ...
        # Grants moved between users across sessions; drop every live cache
    ...
    def set_admin(self, user_id: str, is_admin: bool) -> None:
        ...
        # The admin flag flips access on every session for this user; drop every
        # live cache, since the same row backs the accounts store's view too.
        invalidate_resolve_access_caches()
