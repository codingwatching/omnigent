...
def test_account_store_writes_evict_permission_store_cache(db_uri: str) -> None:
    """The accounts-mode store writes the same rows, so its writes must evict.

    ``SqlAlchemyAccountStore`` is a second writer of ``users`` /
    ``session_permissions`` and holds no reference to the permission store, so
    without the shared invalidation hook a demotion or a user deletion would
    keep being served from this cache for a full TTL.
    """
    ...
    # An admin's access is cached, then the accounts store demotes them.
    _ensure_user(perm_store, "root@test.com")
    perm_store.set_admin("root@test.com", True)
    assert perm_store.resolve_access("root@test.com", conv_id).is_admin is True
    account_store.set_admin("root@test.com", False)
    assert perm_store.resolve_access("root@test.com", conv_id).is_admin is False, (
        "a demotion through the accounts store must not be served from a stale cache"
    )

    # A granted user's access is cached, then the accounts store deletes them.
    _ensure_user(perm_store, "mallory@test.com")
    perm_store.grant("mallory@test.com", conv_id, level=2)
    assert perm_store.resolve_access("mallory@test.com", conv_id).user_grant_level == 2
    assert account_store.delete_user("mallory@test.com") is True
    assert perm_store.resolve_access("mallory@test.com", conv_id).user_grant_level is None, (
        "deleting a user must drop their cached access immediately"
    )
